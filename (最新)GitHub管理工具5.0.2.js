            ...
            return new Promise((resolve, reject) => {
                const options = {
                    ...
                    headers: {
                        ...
                        'User-Agent': 'GitHub-Batch-Tools/5.0.2'
                    },
                    timeout: 30000,
                    onload: (response) => {
...
`;
    ...
    class FileUploadManager {
        constructor(api, repoInfo, branch) {
            ...
            this.folderBasePath = '';
            this.selectedFolderName = ''; // 新增：存储所选文件夹的名称
            this.onFileListChanged = null; // 新增：文件列表变化回调
        }
        ...
        async selectFolders() {
            return new Promise((resolve) => {
                ...
                folderInput.multiple = false; // 只能选择一个文件夹
        ...
        async processSelectedFiles(files, isFolderSelection = false) {
            ...
            for (const file of files) {
                ...
                if (isFolderSelection && file.webkitRelativePath) {
                    // 对于文件夹选择，webkitRelativePath 已经是相对于所选文件夹的路径
                    // 例如: "ziji/vercel.json"
                    // 我们需要保持这个相对路径不变
                    relativePath = file.webkitRelativePath;
                    
                    // 调试信息
                    console.log(`[FileUploadManager] 文件夹选择: 文件名=${file.name}, webkitRelativePath=${file.webkitRelativePath}, 相对路径=${relativePath}`);
                } else {
                    // 对于文件选择，直接使用文件名
            ...
            panel.innerHTML = `
                <div class="github-tools-header">
                    <div class="header-content">
                        ...
                        <span class="title">GitHub 批量工具 v5.0.2</span>
                    </div>
            ...
            const result = await Swal.fire({
                ...
                html: `
                    <div style="text-align: left; font-size: 13px;">
                        ...
                        <p><strong>📌 版本:</strong> 5.0.2 (添加文件选择状态显示)</p>
                        <p><strong>🆕 新功能:</strong></p>
                        <ul style="margin-left: 18px; line-height: 1.6;">
                            <li>✅ 添加已选择文件状态显示</li>
                            <li>✅ 实时显示文件数量、文件夹数量和总大小</li>
                            <li>✅ 支持清除所有已选择的文件</li>
                            <li>✅ 改进用户反馈体验</li>
                        </ul>
                    </div>
    ...
    function main() {
        if (StateManager.isGitHubPage() && StateManager.getScriptEnabled()) {
            console.log('[GitHub Batch Tools] v5.0.2 - 开始初始化');
    ...
    function initializeGitHubPage() {
        ...
        const initializeWithRetry = (retryCount = 0) => {
            try {
                ...
                // 使用MutationObserver监听DOM变化，确保在页面完全加载后初始化
                    ...
                    if (shouldInitialize) {
                        ...
                        setTimeout(() => {
                            try {
                                ...
                                if (!document.getElementById('github-tools-floating')) {
                                    new GitHubUIManager();
                                    console.log('[GitHub Batch Tools] v5.0.2 初始化成功（通过DOM监听）');
                                }
                            } catch (error) {
                ...
                setTimeout(() => {
                    try {
                        ...
                        if (!document.getElementById('github-tools-floating')) {
                            new GitHubUIManager();
                            console.log('[GitHub Batch Tools] v5.0.2 初始化成功');
                        }
    ...
    if (document.readyState === 'loading') {
        ...
        // 使用延迟启动，确保GitHub页面完全加载
        setTimeout(main, 2000);
        
        // 添加一个额外的检查，以防GitHub使用AJAX加载内容
