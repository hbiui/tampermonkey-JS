...
// @version      5.2.0
// @description  在GitHub页面添加批量操作按钮：删除所有文件（保留目录结构）、创建.gitignore文件、上传本地文件到仓库和一键删除存储库 - 支持自动避让、透明度/大小配置、快捷键自定义
// @author       Your Name
...
(function() {
    ...
    const CONFIG = {
        ...
        PANEL_OPEN_KEY: 'panel_open_v52',
        FAB_POSITION_KEY: 'fab_position_v52',
        // 新增配置项
        FAB_OPACITY_KEY: 'fab_opacity_v52',
        FAB_SIZE_KEY: 'fab_size_v52',
        HOTKEY_TOGGLE_KEY: 'hotkey_toggle_v52',
        HOTKEY_DELETE_KEY: 'hotkey_delete_v52',
        HOTKEY_UPLOAD_KEY: 'hotkey_upload_v52',
        // 默认值
        DEFAULT_FAB_OPACITY: 0.9,
        DEFAULT_FAB_SIZE: 56,
        DEFAULT_HOTKEY_TOGGLE: 'Alt+G',
        DEFAULT_HOTKEY_DELETE: 'Alt+D',
        DEFAULT_HOTKEY_UPLOAD: 'Alt+U'
    };
...
.config-input:focus {
    outline: none;
    border-color: #4facfe;
    box-shadow: 0 0 0 3px rgba(79, 172, 254, 0.1), inset 0 2px 4px rgba(0, 0, 0, 0.02);
    background: #f8fbff;
}
...
`;
    ...
    class StateManager {
        ...
        // 获取悬浮按钮透明度
        static getFabOpacity() {
            return GM_getValue(CONFIG.FAB_OPACITY_KEY, CONFIG.DEFAULT_FAB_OPACITY);
        }

        // 设置悬浮按钮透明度
        static setFabOpacity(opacity) {
            GM_setValue(CONFIG.FAB_OPACITY_KEY, opacity);
        }

        // 获取悬浮按钮大小
        static getFabSize() {
            return GM_getValue(CONFIG.FAB_SIZE_KEY, CONFIG.DEFAULT_FAB_SIZE);
        }

        // 设置悬浮按钮大小
        static setFabSize(size) {
            GM_setValue(CONFIG.FAB_SIZE_KEY, size);
        }

        // 获取切换面板快捷键
        static getHotkeyToggle() {
            return GM_getValue(CONFIG.HOTKEY_TOGGLE_KEY, CONFIG.DEFAULT_HOTKEY_TOGGLE);
        }

        // 设置切换面板快捷键
        static setHotkeyToggle(hotkey) {
            GM_setValue(CONFIG.HOTKEY_TOGGLE_KEY, hotkey);
        }

        // 获取删除文件快捷键
        static getHotkeyDelete() {
            return GM_getValue(CONFIG.HOTKEY_DELETE_KEY, CONFIG.DEFAULT_HOTKEY_DELETE);
        }

        // 设置删除文件快捷键
        static setHotkeyDelete(hotkey) {
            GM_setValue(CONFIG.HOTKEY_DELETE_KEY, hotkey);
        }

        // 获取上传文件快捷键
        static getHotkeyUpload() {
            return GM_getValue(CONFIG.HOTKEY_UPLOAD_KEY, CONFIG.DEFAULT_HOTKEY_UPLOAD);
        }

        // 设置上传文件快捷键
        static setHotkeyUpload(hotkey) {
            GM_setValue(CONFIG.HOTKEY_UPLOAD_KEY, hotkey);
        }
    }
            ...
            return new Promise((resolve, reject) => {
                const options = {
                    ...
                    headers: {
                        ...
                        'User-Agent': 'GitHub-Batch-Tools/5.2.0'
                    },
                    timeout: 30000,
                    onload: (response) => {
...
`;
    ...
    class GitHubUIManager {
            ...
            panel.innerHTML = `
                <div class="github-tools-header">
                    <div class="header-content">
                        ...
                        <span class="title">GitHub 批量工具 v5.2.0</span>
                    </div>
                ...
                <div class="github-tools-body">
                    ...
                    <div class="github-tools-section">
                        ...
                        <div class="github-tools-buttons">
                            ...
                            <button class="github-tool-btn settings" id="github-about-btn">
                                💡 关于
                            </button>
                    ...
                    <div class="github-tools-section" id="config-section" style="display: none;">
                        <div class="github-tools-section-title">⚙️ 高级设置</div>
                        <div class="config-group">
                            <label class="config-label">
                                <span>🔍 悬浮按钮透明度</span>
                                <span class="config-value" id="opacity-value">90%</span>
                            </label>
                            <input type="range" class="config-slider" id="opacity-slider" 
                                   min="10" max="100" value="90" step="5">
                        </div>
                        <div class="config-group">
                            <label class="config-label">
                                <span>📏 悬浮按钮大小</span>
                                <span class="config-value" id="size-value">56px</span>
                            </label>
                            <input type="range" class="config-slider" id="size-slider" 
                                   min="40" max="80" value="56" step="4">
                        </div>
                        <div class="config-group">
                            <label class="config-label">⌨️ 切换面板快捷键</label>
                            <input type="text" class="config-input" id="hotkey-toggle" 
                                   placeholder="例如: Alt+G">
                            <div class="config-hint">支持组合键：Ctrl, Alt, Shift + 字母</div>
                        </div>
                        <button class="github-tool-btn" id="save-config-btn" style="width: 100%; margin-top: 10px;">
                            💾 保存设置
                        </button>
                    </div>
                </div>
        ...
        bindPanelEvents(panel) {
            ...
            panel.addEventListener('click', (e) => {
                ...
                } else if (target.id === 'github-about-btn') {
                    e.preventDefault();
                    e.stopPropagation();
                    this.showAbout();
        ...
        createFloatingButton() {
            ...
            // 获取保存的配置
            const opacity = StateManager.getFabOpacity();
            const size = StateManager.getFabSize();
            ...
            fab.addEventListener('mouseleave', () => {
                fab.style.transform = 'scale(1)';
                        ...
                        // v5.2.0: 悬浮按钮可拖动 + 优化拖动体验
            let dragging = false, sx = 0, sy = 0, hasMoved = false;
            ...
            document.addEventListener('mousemove', (e) => {
                ...
                if (hasMoved) {
                    ...
                    animationFrame = requestAnimationFrame(() => {
                        ...
                        // 计算新位置
                        const newLeft = Math.min(window.innerWidth - size, Math.max(0, rect.left + (e.clientX - sx)));
                        const newTop = Math.min(window.innerHeight - size, Math.max(0, rect.top + (e.clientY - sy)));
            ...
            document.addEventListener('mouseup', () => {
                ...
                // 应用惯性效果
                const inertiaStrength = 100;
                const friction = 0.95;
                let vx = velocityX * inertiaStrength;
                let vy = velocityY * inertiaStrength;
            ...
            document.addEventListener('touchmove', (e) => {
                ...
                if (hasMoved) {
                    ...
                    animationFrame = requestAnimationFrame(() => {
                        ...
                        if (timeDelta > 0) {
                            velocityX = (touch.clientX - lastX) / timeDelta;
                            velocityY = (touch.clientY - lastY) / timeDelta;
                        }
                        
                        const newLeft = Math.min(window.innerWidth - size, Math.max(0, rect.left + (touch.clientX - sx)));
                        const newTop = Math.min(window.innerHeight - size, Math.max(0, rect.top + (touch.clientY - sy)));
        ...
        setupEnhancedHotkeys() {
            document.addEventListener('keydown', (e) => {
                ...
                // 获取配置的快捷键
                const hotkeyToggle = StateManager.getHotkeyToggle();
                const hotkeyDelete = StateManager.getHotkeyDelete();
                const hotkeyUpload = StateManager.getHotkeyUpload();

                // 切换面板 (默认 Alt+G)
                if (this.matchHotkey(e, hotkeyToggle)) {
                    e.preventDefault();
                    this.togglePanel();
                }

                // 删除文件 (默认 Alt+D)
                if (this.matchHotkey(e, hotkeyDelete)) {
                    e.preventDefault();
                    this.handleDeleteAllFiles();
                }

                // 上传文件 (默认 Alt+U)
                if (this.matchHotkey(e, hotkeyUpload)) {
                    e.preventDefault();
        ...
        setupOverlayDetection() {
            const checkInterval = setInterval(() => {
                ...
                const overlays = this.detectGitHubOverlays();
                if (overlays.length > 0) {
                    const panelRect = panel.getBoundingClientRect();
                    for (const overlay of overlays) {
                        if (this.isOverlapping(panelRect, overlay.rect)) {
                            // 发生重叠,调整位置
                            this.adjustPanelPosition(panel, overlay.rect);
                            break;
                        }
                    }
                }
        ...
        // 检测GitHub原生浮层
        detectGitHubOverlays() {
            const overlays = [];
            
            // 通知面板
            const notif = document.querySelector('.notifications-list-container, [data-target="notifications-list.container"]');
            if (notif && notif.offsetParent !== null) {
                overlays.push({ element: notif, rect: notif.getBoundingClientRect(), type: 'notification' });
            }

            // 用户菜单
            const menu = document.querySelector('details-menu[role="menu"]');
            if (menu && menu.offsetParent !== null) {
                overlays.push({ element: menu, rect: menu.getBoundingClientRect(), type: 'menu' });
            }

            // 搜索建议
            const search = document.querySelector('.search-suggestions, .jump-to-suggestions');
            if (search && search.offsetParent !== null) {
                overlays.push({ element: search, rect: search.getBoundingClientRect(), type: 'search' });
            }
        ...
        // 调整面板位置以避让浮层
        adjustPanelPosition(panel, overlayRect) {
            const panelRect = panel.getBoundingClientRect();
            const viewportWidth = window.innerWidth;
            const viewportHeight = window.innerHeight;
            
            // 尝试向左移动
            let newLeft = overlayRect.left - panelRect.width - 30;
            if (newLeft < 20) {
                // 左侧空间不足,尝试向下移动
                const newTop = overlayRect.bottom + 20;
                if (newTop + panelRect.height < viewportHeight - 20) {
                    panel.style.top = newTop + 'px';
                    panel.style.left = overlayRect.left + 'px';
                } else {
                    // 向上移动
                    panel.style.top = Math.max(20, overlayRect.top - panelRect.height - 20) + 'px';
                }
            } else {
            ...
            const result = await Swal.fire({
                ...
                html: `
                    <div style="text-align: left; font-size: 13px;">
                        ...
                        <hr style="margin: 12px 0; border: none; border-top: 1px solid #e9ecef;">
                        <p><strong>📌 版本:</strong> 5.2.0 (修复版 - 配置面板可用)</p>
                        <p><strong>🆕 本次更新:</strong></p>
                        <ul style="margin-left: 18px; line-height: 1.6;">
                            <li>✅ 修复配置面板无法打开的问题</li>
                            <li>✅ 透明度和大小设置可立即生效</li>
                            <li>✅ 增加独立的"关于"按钮显示版本和Token信息</li>
                            <li>✅ 优化设置面板的交互体验</li>
                        </ul>
                        <p><strong>⌨️ 快捷键:</strong> 可在"⚙️ 设置"中自定义</p>
        ...
        loadConfig() {
            ...
            const toggleInput = document.getElementById('hotkey-toggle');
            const deleteInput = document.getElementById('hotkey-delete');
            const uploadInput = document.getElementById('hotkey-upload');

            if (toggleInput) toggleInput.value = hotkeyToggle;
            if (deleteInput) deleteInput.value = hotkeyDelete;
            if (uploadInput) uploadInput.value = hotkeyUpload;
        }
        ...
        saveConfig() {
            ...
            const opacitySlider = document.getElementById('opacity-slider');
            if (opacitySlider) {
                StateManager.setFabOpacity(opacitySlider.value / 100);
            }

            // 保存大小
            const sizeSlider = document.getElementById('size-slider');
            if (sizeSlider) {
                StateManager.setFabSize(parseInt(sizeSlider.value));
            }
            ...
            if (toggleInput && toggleInput.value.trim()) {
                StateManager.setHotkeyToggle(toggleInput.value.trim());
            }
            if (deleteInput && deleteInput.value.trim()) {
                StateManager.setHotkeyDelete(deleteInput.value.trim());
            }
            if (uploadInput && uploadInput.value.trim()) {
                StateManager.setHotkeyUpload(uploadInput.value.trim());
            }
        ...
        async deleteAllFiles() {
            ...
            try {
                const progressSwal = Swal.fire({
                    ...
                    html: `
                        <div style="text-align: center;">
                            ...
                            <div id="swal-status-text" class="status-text" style="font-size: 12px;">正在获取所有内容...</div>
                        </div>
                    `,
                    allowOutsideClick: false,
                    showConfirmButton: false,
                ...
                // 获取所有文件（这会递归获取所有目录下的文件）
                updateProgress(5, '正在扫描仓库...');
                ...
                // 删除所有文件
                updateProgress(10, `找到 ${files.length} 个文件，开始删除...`);

                for (let i = 0; i < files.length; i++) {
                    const file = files[i];
                    const percent = 10 + Math.round(((i + 1) / files.length) * 80);

                    updateProgress(percent, `正在删除文件 ${i + 1}/${files.length}: ${file.name}...`);
                ...
                // 所有文件删除后，GitHub会自动删除空文件夹
                // 但如果有.gitignore等隐藏文件，需要再次扫描并删除
                updateProgress(95, '检查剩余内容...');
                await Utils.delay(1000);

                const remainingFiles = await this.operations.getAllFiles().catch(() => []);
                
                if (remainingFiles.length > 0) {
                    updateProgress(95, `发现剩余 ${remainingFiles.length} 个文件，继续删除...`);
                    ...
                    for (let i = 0; i < remainingFiles.length; i++) {
                        ...
                        if ((i + 1) % 3 === 0 && i < remainingFiles.length - 1) {
                            await Utils.delay(800);
                        }
                    }
                }
                ...
                let resultHtml = `<strong>所有内容删除操作完成！</strong><br><br>`;
                resultHtml += `✅ 成功删除: <strong>${successCount}</strong> 个文件<br>`;
                resultHtml += `❌ 删除失败: <strong>${failCount}</strong> 个文件<br><br>`;
                resultHtml += `<small style="color: #6c757d;">📁 空文件夹已被 GitHub 自动清理</small>`;
                ...
                console.error('[GitHubUIManager] 删除所有内容失败:', error);
                Swal.close();
    ...
    function createControlPanel(container) {
        ...
        panel.innerHTML = `
            ...
            <div style="margin-top: 16px; padding: 12px; background: linear-gradient(145deg, #f8f9fa 0%, #ffffff 100%); border-radius: 10px; font-size: 11px; color: #666; line-height: 1.6;">
                ...
                <br>
                <small>📁 <strong>更新 v5.2.0:</strong> 修复子目录上传问题 - 正确解析分支和路径</small>
                <br>
    ...
    function main() {
        if (StateManager.isGitHubPage() && StateManager.getScriptEnabled()) {
            console.log('[GitHub Batch Tools] v5.2.0 - 开始初始化');
            ...
            try {
                    ...
                    if (shouldInitialize) {
                        ...
                        setTimeout(() => {
                            try {
                                if (!document.getElementById('github-tools-floating')) {
                                    new GitHubUIManager();
                                    console.log('[GitHub Batch Tools] v5.2.0 初始化成功（通过DOM监听）');
                                }
                            } catch (error) {
                ...
                setTimeout(() => {
                    try {
                        if (!document.getElementById('github-tools-floating')) {
                            new GitHubUIManager();
                            console.log('[GitHub Batch Tools] v5.2.0 初始化成功');
                        }
                        observer.disconnect();
                    } catch (error) {
                        console.error('[GitHub Batch Tools] 脚本初始化失败:', error);
