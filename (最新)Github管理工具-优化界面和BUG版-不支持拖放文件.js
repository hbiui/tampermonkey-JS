...
(function() {
    ...
    const CONFIG = {
        ...
        GITHUB_TOKEN_KEY: 'github_token_v4',
        SCRIPT_ENABLED_KEY: 'script_enabled_v4',
        PANEL_POSITION_KEY: 'panel_position_v4',
        MAX_RETRIES: 3,
        RETRY_DELAY: 1000,
        PAGE_SIZE: 100,
        CHUNK_SIZE: 3,
            ...
            return new Promise((resolve, reject) => {
                const options = {
                    ...
                    headers: {
                        ...
                        'User-Agent': 'GitHub-Batch-Tools/4.0.0'
                    },
                    timeout: 30000,
                    onload: (response) => {
                        ...
                        if (response.status >= 200 && response.status < 300) {
                            try {
                                if (method === 'DELETE' && (!response.responseText || response.responseText === '')) {
...
`;
    ...
    class FileUploadManager {
        ...
        async handleDropItems(items) {
            ...
            for (const item of items) {
                ...
                if (entry) {
                    const files = await this.getFolderFiles(entry);
                    for (const fileInfo of files) {
                        if (fileInfo.size > CONFIG.MAX_FILE_SIZE) {
                            console.warn(`[FileUploadManager] 文件 ${fileInfo.name} 超过 ${CONFIG.MAX_FILE_SIZE / (1024 * 1024)}MB 限制，跳过`);
                            continue;
                        }
        ...
        async handleConflicts(conflicts) {
            ...
            const conflictListHtml = conflicts.slice(0, 10).map((conflict, index) => {
                return `<div style="padding: 6px; background: rgba(220, 53, 69, 0.05); border-radius: 4px; margin-bottom: 4px;">
                    <strong>${index + 1}.</strong> ${conflict.path}
                </div>`;
            }).join('');
    ...
    class GitHubUIManager {
            ...
            panel.innerHTML = `
                <div class="github-tools-header">
                    <div class="header-content">
                        ...
                        <span class="title">GitHub 批量工具</span>
                    </div>
            ...
            const result = await Swal.fire({
                ...
                html: `
                    <div style="text-align: left; font-size: 13px;">
                        ...
                        <p><strong>📌 版本:</strong> 4.0.0 (优化版)</p>
                    </div>
                `,
                showDenyButton: true,
                showCancelButton: true,
            ...
            const { isConfirmed } = await Swal.fire({
                ...
                html: `
                    <div style="text-align: left; font-size: 13px;">
                        ...
                        <p style="color: #dc3545; font-weight: 600; margin-top: 12px;">⚠️ 此操作将删除仓库中的所有文件！</p>
                        <p>请在下方输入仓库名称以确认:</p>
                        <input type="text" id="confirm-repo-delete" class="swal2-input" placeholder="${repoName}" autocomplete="off" style="font-size: 13px;">
                    </div>
                `,
                icon: 'warning',
                showCancelButton: true,
                ...
                width: 450,
                preConfirm: () => {
                    const input = document.getElementById('confirm-repo-delete');
        ...
        async deleteFilesAndKeepStructure() {
            ...
            const result = await Swal.fire({
                ...
                html: `
                    <div style="text-align: left; font-size: 13px;">
                        ...
                        <ol style="margin-left: 18px; margin-bottom: 10px; line-height: 1.8;">
                            <li>删除仓库中的所有文件（保留目录结构）</li>
                            <li>在所有目录中创建 .gitignore 文件来保持目录结构</li>
                        </ol>
                        <p style="color: #fd7e14; font-weight: 600;">⚠️ 注意: 如果目录已存在 .gitignore 文件，将不会重复创建。</p>
                        <p style="color: #fd7e14; font-weight: 600;">⚠️ 此操作将修改仓库内容，请谨慎操作。</p>
                        <p>请在下方输入仓库名称以确认:</p>
            ...
            try {
                ...
                if (hasFileFailures || hasGitignoreFailures) {
                    resultHtml += `<br><details style="text-align: left;"><summary>📋 查看失败详情</summary>`;
                    resultHtml += `<div class="error-details">`;
        ...
        async uploadFiles() {
            ...
            const { value: uploadType, isDismissed } = await Swal.fire({
                title: '📤 上传文件',
                html: `
                    <div style="text-align: center; padding: 20px;">
                        <div style="font-size: 48px; margin-bottom: 16px;">📤</div>
                        <p style="margin-bottom: 20px; color: #666;">请选择上传方式</p>
                    </div>
                `,
                showCancelButton: true,
            ...
            try {
                ...
                const { value: confirmUpload, isDismissed: uploadDismissed } = await Swal.fire({
                    ...
                    html: `
                        <div style="text-align: left; font-size: 13px;">
                            <div class="file-stats" style="margin-bottom: 16px;">
                                <div class="file-stat">
                                    <div class="file-stat-value">${files.length}</div>
                                    <div class="file-stat-label">文件数</div>
                                </div>
                                <div class="file-stat">
                                    <div class="file-stat-value">${stats.folders}</div>
                                    <div class="file-stat-label">文件夹</div>
                                </div>
                                <div class="file-stat">
                                    <div class="file-stat-value">${Utils.formatFileSize(stats.totalSize)}</div>
                                    <div class="file-stat-label">总大小</div>
                                </div>
                            </div>
                    ...
                    didOpen: () => {
                        ...
                        document.querySelectorAll('.remove-file').forEach(btn => {
                            btn.addEventListener('click', (e) => {
                                ...
                                const statElements = document.querySelectorAll('.file-stat-value');
        ...
        async deleteRepository() {
            ...
            const warningResult = await Swal.fire({
                ...
                html: `
                    <div style="text-align: left; font-size: 13px;">
                        <p style="color: #dc3545; font-weight: bold; margin-bottom: 12px; font-size: 15px;">🔥 您将要永久删除存储库！</p>
                        <div class="danger-note">
                            <p style="font-weight: 600; margin-bottom: 8px;">删除存储库将导致：</p>
                            <ul style="margin: 8px 0; padding-left: 18px;">
                                <li>所有文件、文件夹被永久删除</li>
                                <li>所有提交历史、分支、标签丢失</li>
                                <li>所有 issues、pull requests 被删除</li>
                                <li>所有协作者将失去访问权限</li>
                                <li style="font-weight: 600; color: #dc3545;">⚠️ 此操作无法撤销！</li>
                            </ul>
                        </div>
                        <p>请在下方输入 <strong style="color: #dc3545; font-size: 16px;">DELETE</strong> 以确认您了解此操作的严重性：</p>
                        <input type="text" id="confirm-danger" class="swal2-input" placeholder="输入 DELETE" autocomplete="off" style="font-size: 14px; font-weight: 600;">
                    </div>
                `,
                icon: 'error',
                showCancelButton: true,
                confirmButtonText: '🗑️ 继续删除',
                cancelButtonText: '❌ 取消',
                confirmButtonColor: '#dc3545',
                focusCancel: true,
                width: 550,
                ...
                const repoDetailsHtml = `
                    <div style="text-align: left; font-size: 13px;">
                        ...
                        <div class="danger-note">
                            ...
                            <input type="text" id="confirm-repo-fullname" class="swal2-input" placeholder="${repositoryData.full_name}" autocomplete="off" style="font-size: 13px; font-weight: 600;">
                        </div>
                    </div>
                `;
    ...
    function main() {
        if (StateManager.isGitHubPage() && StateManager.getScriptEnabled()) {
            console.log('[GitHub Batch Tools] v4.0.1 - 开始初始化');
        ...
        const initializeWithRetry = (retryCount = 0) => {
            try {
                ...
                const observer = new MutationObserver((mutations) => {
                    ...
                    if (shouldInitialize) {
                        ...
                        setTimeout(() => {
                            try {
                                new GitHubUIManager();
                                console.log('[GitHub Batch Tools] v4.0.1 初始化成功（通过DOM监听）');
                            } catch (error) {
                ...
                setTimeout(() => {
                    try {
                        new GitHubUIManager();
                        console.log('[GitHub Batch Tools] v4.0.1 初始化成功');
                        observer.disconnect(); // 初始化成功后停止观察
                    } catch (error) {
                        console.error('[GitHub Batch Tools] 脚本初始化失败:', error);
                    }
                }, 1000);
